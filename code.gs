// ═══════════════════════════════════════════════════════════════
//  NEXUS · RescueTap Operations — Apps Script Backend v3.1
//  Fixes from v3.0:
...
const FOUNDER_EMAIL  = 'othnielemry@gmail.com';              // ← Ayo T. Joshua's email (google login resolves this to Ayo via the '9. Team Members' roster, not Oye — fixed mislabeled comment)
const COO_EMAIL      = 'abionnexus@gmail.com';           // ← Shina Samson (test inbox)
const DEPUTY_EMAIL   = 'othnielemrys@gmail.com';         // ← Ayo T. Joshua (test inbox)
const TEAM_LEAD_EMAIL= 'othnielemry@gmail.com'; // ← team lead email (update if multiple)
...
const COO_NAME       = 'Shina Samson';
const DEPUTY_NAME    = 'Ayo T. Joshua';
const TEAM_LEAD_NAME = 'Ayo T. Joshua';
const APP_LINK       = 'https://nexus-rct.vercel.app/';
const SHEET_LINK     = 'https://docs.google.com/spreadsheets/d/' + SS.getId();
const RESEND_FROM    = 'NEXUS · RescueTap <nexus@mail.newageskills.com.ng>';
...
function sendViaGmailFallback(toList, subject, htmlBody, plainBody, options) {
  ...
  try {
    ...
    const gmailOptions = { name: 'NEXUS · RescueTap', htmlBody: htmlBody || undefined };
    if (options.cc) {
      const cc = Array.isArray(options.cc) ? options.cc.filter(Boolean) : [options.cc].filter(Boolean);
...
const LEADERS  = ['bowmandan','ayo','shina'];
const MANAGERS = ['bowmandan','ayo','shina'];
// Was: a hardcoded array, frozen at whatever the team looked like when it
// was written — confirmed stale (missing "Daniel Bowman", who's on the
// live roster) and would silently miss anyone added later too. Used in
...
const NAME_ALIASES = {
  ...
  // othnielemry@gmail.com is Ayo's designated leader inbox (see
...
// "Ayo" / "Ayo and Chidi" / "Ayo, Chidi and Seth"
...
const TRUSTED_EDITORS = [
  'Chizaram',  // can edit leader-only fields without leader login
...
// (kept for back-compat with existing grants like Chizaram's), or a
...
const TAB_HEADERS = {
  ...
  '9. Team Members'        : ['Name','Email','Role','Department','EOD_Days','Notes','Trusted_MasterTasks','Trusted_COS','Trusted_Partnerships','Trusted_NewButton','Picture','Status'],
  ...
  '16. Pending AI Tasks'   : [
    ...
    'Suggested Deadline','Priority','Status','Source','Created_At','Reviewed_By','Reviewed_At'
  ],
...
function sendStateSecurityReport(state, token) {
  ...
  const subject = '[RescueTap] Security & public-safety scan — ' + state;
  const body =
    ...
    `— NEXUS Automation · RescueTap Ops`;
  try {
  ...
  if (isAutomationEnabled('calendar_sync') && data.Date && data.Title && data.Location_Type !== 'Physical') {
    try {
      ...
      const event     = CalendarApp.getDefaultCalendar().createEvent(
        '[RescueTap] ' + data.Title, startDate, endDate,
...
function sendChatMessage(data, token) {
  ...
  } catch(pe) { Logger.log('broadcastPush error: ' + pe.message); }
...
function hasTaskThreadAccess(identity, taskId) {
  ...
  // person on a multi-assignee task ("Ayo T. Joshua, Titi"), and Support
...
function getChatSummary(token) {
  ...
  // contributing to the "you have unread messages" badge.
...
// Returns every task-chat thread that has at least one message, with task
// title/assignee/unread count/last message, so the frontend can render a
// "browse all task chats" list and a "My Task Chats" filter (threads where
// I'm the assignee, or I've already posted in that thread).
function getChatThreads(token) {
  ...
  const threads = Object.keys(base).map(taskId => {
    ...
    return {
      ...
      isMine: !!assignee && assignee === me || participants.indexOf(me) !== -1,
    };
...
function sendTeamInviteEmail(name, email, invitedBy) {
  try {
    ...
    const subject = 'You\'ve been invited to NEXUS · RescueTap';
    const html =
      '<p>Hi ' + name + ',</p>' +
      '<p>' + invitedBy + ' added you to NEXUS, RescueTap\'s internal operations app. ' +
    ...
    const plain = 'Hi ' + name + ',\n\n' + invitedBy + ' added you to NEXUS, RescueTap\'s internal operations app.\n' +
...
function enforceLeaderOnlyFields(data, token, tabName) {
  ...
  // trusted-but-non-leader editor (e.g. Chizaram via TRUSTED_EDITORS) could
...
function sendAssignmentEmail(task) {
  ...
  const subject = `[RescueTap] New Task Assigned: "${taskDesc}"`;
  ...
  var htmlBody =
    ...
    '<span style="font-size:12px;font-weight:700;color:rgba(255,255,255,0.85);margin-left:10px;font-family:monospace">RescueTap Operations</span>' +
    '</div>' +
    ...
    '<p style="margin:0;font-size:11px;color:#999;font-family:monospace">— NEXUS Automation · RescueTap Ops · Task ' + taskId + '</p>' +
    '</div></div>';
  ...
  var plainBody =
    ...
    (ccNotePlain ? '\n\n(' + ccNotePlain + ')' : '') +
    '\n\n— NEXUS Automation · RescueTap Ops · Task ' + taskId;

  const delivered = sendViaResend(assigneeEmails, subject, htmlBody, plainBody, {
...
function addDeadlineToCalendar(task, assigneeEmails) {
  ...
  const title = `🔴 [RescueTap] DEADLINE: ${taskDesc}`;
...
function sendDeadlineEmail(task, assigneeEmails) {
  ...
  const subject = `[RescueTap] Deadline Reminder: "${taskDesc}" — ${deadline}`;
  var prioColor = priority === 'URGENT' ? '#f05060' : priority === 'HIGH' ? '#f0a030' : priority === 'MEDIUM' ? '#f0c040' : '#8bc34a';
  ...
  sendViaResend(assigneeEmails, subject, htmlBody, plainBody, {
    replyTo: FOUNDER_EMAIL,
    cc: ccAll,
  });
...
function sendMondayDigest() {
  ...
  Object.entries(byAssignee).forEach(([email, { name, tasks: myTasks }]) => {
    ...
    const subject = `[RescueTap] Your Week Ahead — ${myTasks.length} Active Task(s) · ${today.toDateString()}`;
    const body =
      `Good morning ${name},\n\n` +
    ...
    try {
      sendViaResend(email, subject, null, body, { replyTo: FOUNDER_EMAIL });
      Logger.log('Monday digest sent to ' + email);
...
function sendDailyMorningDigest() {
  ...
  Object.entries(byAssignee).forEach(([email, { name, tasks: myTasks }]) => {
    ...
    const subject = `[RescueTap] Good ${dayNames[day]} ${name.split(' ')[0]} — ${myTasks.length} active task(s)`;
    const body =
      `Good morning ${name},\n\n` +
    ...
    try {
      sendViaResend(email, subject, null, body, { replyTo: FOUNDER_EMAIL });
      Logger.log('Morning digest sent to ' + email);
...
function sendCOBReminders() {
  ...
  pending.forEach(memberName => {
    ...
    if (!email) return;
    const subject = `[RescueTap] ⏰ Don't forget your EOD report — ${today.toDateString()}`;
    const body =
    ...
    try {
      sendViaResend(email, subject, null, body, { replyTo: FOUNDER_EMAIL });
      Logger.log('COB reminder sent to ' + email);
...
function runHourlyFollowUps() {
  ...
  tasks.forEach(task => {
    ...
    const subject = `[RescueTap] ⏱ ${Math.round(hoursSince)}h check-in: "${task['Task Description']}"`;
    const body =
...
function updateRow(tabName, id, data, token) {
  ...
  if (OWNERSHIP_TABS.includes(tabName)) {
    ...
    if (!isLeaderCaller && !isTrusted) {
      ...
      // failed for multi-assignee tasks ("Ayo T. Joshua, Titi") and for
  ...
  tasks.forEach(task => {
    ...
    if (tier === 1) {
      ...
      const body =
        ...
        `— NEXUS Automation · RescueTap Ops`;
      const cc2day = [TEAM_LEAD_EMAIL].filter(e=>e&&!e.includes('your_')).join(',');
      try { sendViaResend(assigneeEmail, subject, null, body, { replyTo: FOUNDER_EMAIL, cc: cc2day || null }); } catch(e) { Logger.log('tier1 email error: '+e.message); }
    ...
    if (tier === 2) {
      ...
      const body =
        ...
        `— NEXUS Automation · RescueTap Ops`;
      try {
        sendViaResend(assigneeEmail, subject, null, body, {
          replyTo: FOUNDER_EMAIL,
          cc: [TEAM_LEAD_EMAIL, COO_EMAIL, DEPUTY_EMAIL].filter(e=>e&&!e.includes('your_')),
        });
    ...
    if (tier === 3) {
      ...
      try {
        sendViaResend(FOUNDER_EMAIL, subject, null, body, {
          cc: [assigneeEmail, COO_EMAIL, DEPUTY_EMAIL].filter(e=>e&&!e.includes('your_')),
        });
      } catch(e) { Logger.log(e.message); }
      pushNames = [FOUNDER_NAME, assigneeName, COO_NAME, DEPUTY_NAME];
    }

    // Push layer — same widening audience as the email, reusing the
    // existing chat-notification push infra rather than a second mechanism.
    const uniqueNames = Array.from(new Set(pushNames.map(n => canonicalizeName(String(n)).trim())));
    broadcastPushTo(uniqueNames, {
  ...
  partners.forEach(p => {
    ...
    if (daysLate === 1 || daysLate === 3 || daysLate === 7) {
      ...
      const body =
        ...
        `Update in NEXUS:\n${SHEET_LINK}\n\n` +
        `— NEXUS Automation · RescueTap Ops`;
      try { sendViaResend(FOUNDER_EMAIL, subject, null, body); } catch(e) { Logger.log(e.message); }
      // Push layer — reuses the same escalation push infra as Master Tasks,
      // ladder/audience otherwise untouched per spec (Founder only).
      broadcastPushTo([canonicalizeName(FOUNDER_NAME)], {
...
// Shina = Mon–Fri (all days). Ayo = Mon, Tue, Thu, Fri (not Wed).
// Add other names with their days below (0=Sun,1=Mon,...,5=Fri,6=Sat).
const EOD_SCHEDULE = {
  'Shina Samson'  : [1,2,3,4,5],  // Mon-Fri
...
function checkDeadlinesAndNotify(force) {
  ...
  tasks.forEach(task => {
    ...
    if (body) {
      try { sendViaResend(task['Assigned_To_Email'], subject, null, body); } catch(e) { Logger.log(e.message); }
    }
  });
...
function createCalendarEvents(callerEmail) {
  ...
  tasks.forEach(task => {
    ...
    try {
      ...
      const title    = `[RescueTap] ${task['Task Description']}`;
...
function _createSingleCalendarEvent(taskId, description, deadlineStr, email) {
  try {
    ...
    const event    = CalendarApp.getDefaultCalendar()
      .createEvent('[RescueTap] ' + description, deadline, new Date(deadline.getTime()+3600000));
